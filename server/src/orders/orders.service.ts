...
export class OrdersService {
    constructor(
        ...
        @InjectModel("OrderProblems") private orderProblemModel: Model<OrderProblem>,
    ...
    async getAll(paginationDTO: AllOrdersDTO): Promise<Record<string, any>> {
        ...
        let sortParameter = allowedParameters.includes(paginationDTO.sortField) ? paginationDTO.sortField : 'orderId';
        ...
        sortParameter = paginationDTO.sortParameter == 'descending' ? '-' + sortParameter : sortParameter;
        ...
        if (paginationDTO.search) {
            ...
            filter['$or'].push({['client.clientFirstName']: {"$regex": paginationDTO.search, "$options": "i"}});
            filter['$or'].push({['client.clientLastName']: {"$regex": paginationDTO.search, "$options": "i"}});
            filter['$or'].push({['totalPrice']: +paginationDTO.search ? +paginationDTO.search : 0});
        }
        ...
        return this.orderModel.find(filter).skip(+paginationDTO.offset)
            .limit(+paginationDTO.limit)
            .sort(sortParameter);
    }

    async getOrders(token: string, paginationDTO?: PaginationDTO): Promise<Record<string, any>> {
        ...
        return this.orderModel.find({'client.clientId': verified.userID})
    ...
    async addOrder(token: string, userDTO: AddOrderDTO) {
        ...
        const newProducts = await Promise.all(userDTO.products.map(async (el)=>{

            const product = await Item.findOne({itemID: el.itemID})
            const variant = product.variants.find((variant) => {return variant.variantID == el.variantID})
            el = {
                itemID: el.itemID,
    ...
    async deleteOrder(userDTO: number) {
        ...
        let order, userId, networkUserId;
        users.forEach((user) => {
    ...
    async addProblem(token: string, userDTO: AddProblemDTO) {
        ...
        if (order.client.clientId != verified.userID) {
            throw new HttpException('You haven`t order with this Id', HttpStatus.BAD_REQUEST)
        }
